...
> **Implementation note:** the current CareFormsML editor uses the compact,
> line-oriented syntax generated by the Visual Form Designer. The XML examples
> below document the original design direction; they are not accepted by the
> current parser.

## Current conditional visibility syntax

CareFormsML represents schema v1 `logic.showWhen` rules with `show_when`.
The statement immediately follows the section or field it controls:

```text
section "Pain assessment" id="pain-assessment"
...
Supported operators are `equals`, `notEquals`, `contains`, `isEmpty`, and
`isNotEmpty`. The first three require `value`; `isEmpty` and `isNotEmpty`
must omit it:

```text
field "Pain notes" id="pain_notes" type=textarea
  show_when field="pain_score" operator=isNotEmpty
```

Quoted comparison values remain strings. Unquoted booleans and numbers retain
their canonical types:

- `value="1"` stores the string `"1"`.
- `value=1` stores the number `1`.
- `value=true` stores the boolean `true`.

Rules may reference fields declared later in the document. References are
